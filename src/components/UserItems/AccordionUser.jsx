    ...
    <>
       <Accordion allowToggle>
                            <AccordionItem bgColor="#d0b08a">
                                <h2>
                                    <AccordionButton>
                                        <Box as='span' textAlign='left' bgColor="var(--nsr-color21)" 
                                        borderBottom="2px solid red"
                                            w="100%" fontSize={{ base: "sm", md: "md", lg: "lg" }} 
                                ...
                                <AccordionPanel mt="-10px">
                                    {(!userMovies.length) && (
                                        ...
                                        <Text color="var(--nsr-color6)"
                                         fontWeight="var(--nsr-fweight1)">You have not added any movie yet</Text>
                                        </Flex>    
                                    )}
                                    {
                                    userMovies?.map((movie, index) => (
                                        <Flex key={index} align="center" justifyContent="space-between"
                                         borderBottom="1px solid red" w="99%"   
                                         p={{ base: "var(--nsr-padding1)", md: "var(--nsr-padding2)" }}>
                                            <Image src={movie.image} 
                                             w={{ base: "30px", md: "70px" }}
                                             h={{ base: "45px", md: "90px" }}
                                            borderRadius="20%" mb="var(--nsr-margin1)" mt="var(--nsr-margin1)" />
                                            <Text color="var(--nsr-color6)" 
                                            fontWeight="var(--nsr-fweight1)" 
                                            fontSize={["8px", "15px", "15px", "15px"]}>{movie.name}</Text>
                                            
                                            
                                      
                                            <DeleteIcon cursor="var(--nsr-cursor1)" 
